...
### 8. User Location & Alert Preferences (Proximity Early Warnings)

- **Endpoint**: `POST /auth/location`
- **Method**: `POST`
- **Headers**: `Authorization: Bearer <token>`
- **Frontend Usage**: Triggered after login/signup when user grants location permission, or updates alert radius in Alert Settings.
- **Request JSON Schema**:
```json
{
  "latitude": 13.0827,
...
### 9. Proximity Disaster Alerts & Hazard Radius

- **Endpoint**: `GET /api/alerts/proximity` (or `POST /api/alerts/proximity`)
- **Method**: `GET`
- **Query Params**: `lat=13.0827&lng=80.2707&radius_km=50`
- **Frontend Usage**: Fetches active hazard alerts within the user's configured proximity radius.
- **Response JSON Schema (`200 OK`)**:
```json
{
  "alerts": [
...
3. **Privacy by Design**: User coordinates are processed strictly for proximity distance calculation and are never stored without consent or exposed publicly.
